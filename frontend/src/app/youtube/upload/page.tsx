...
import { youtubeStudioApi, type StudioCategory } from "@/lib/api";

// v1.1.31: 파이프라인을 거치지 않고 사용자가 직접 영상 파일을 업로드.
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <p className="text-gray-400 text-sm mb-6">
        LongTube 파이프라인을 거치지 않고 가지고 있는 영상 파일을 바로 올립니다. 전부 resumable 업로드로 처리됩니다.
      </p>
      ...
      {result && (
        <div className="bg-green-500/10 border border-green-500/30 text-green-300 text-sm rounded p-4 mb-4">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            <Link href={`/youtube/videos/${result.video_id}`} className="underline">
              편집 페이지로 →
            </Link>
          </div>
        </div>
      ...
      <div className="bg-bg-secondary border border-border rounded-lg p-5 space-y-4">
        ...
        >
          <Upload size={16} /> {uploading ? "업로드 중... (큰 파일은 오래 걸립니다)" : "업로드"}
        </button>
      </div>
    </div>
  );
