...
export default function PlaylistsPage() {
  ...
  const load = async () => {
    ...
    setErr(null);
    try {
      const res = await youtubeStudioApi.listPlaylists(projectId);
      setItems(res.items || []);
    } catch (e) {
  ...
  const saveEdit = async () => {
    ...
    try {
      await youtubeStudioApi.updatePlaylist(
        editing.playlist_id,
        {
          title: editing.title,
  ...
  const del = async (playlist: StudioPlaylist) => {
    if (!projectId || !confirm(`"${playlist.title}" 재생목록을 삭제하시겠습니까? 복구할 수 없습니다.`)) return;
    try {
      await youtubeStudioApi.deletePlaylist(playlist.playlist_id, projectId);
      load();
    } catch (e) {
    ...
    <div className="p-8 max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">선택된 프리셋의 재생목록만 관리합니다.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={!projectId}
            className="bg-accent-primary hover:bg-purple-600 text-white px-3 py-1.5 rounded text-sm flex items-center gap-1 disabled:opacity-50"
          >
            <Plus size={14} /> 새 재생목록
          </button>
        </div>
        ...
        <>
          {showCreate && (
            <div className="bg-bg-secondary border border-border rounded-lg p-4 mb-4">
              <h3 className="text-sm font-semibold mb-3">재생목록 만들기</h3>
              <input
                type="text"
                value={newTitle}
          ...
          <div className="grid grid-cols-2 gap-3">
            {loading && items.length === 0 ? (
              ...
              items.map((playlist) => (
                <div key={playlist.playlist_id} className="bg-bg-secondary border border-border rounded-lg p-4">
                  {editing?.playlist_id === playlist.playlist_id ? (
                    <div>
                      <input
                        type="text"
                        value={editing.title}
                      ...
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => setEditing(null)} className="text-xs text-gray-400 px-2 py-1">
                          취소
                        </button>
                        <button onClick={saveEdit} className="bg-accent-primary text-white px-3 py-1 rounded text-xs">
                          저장
                        </button>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <Link
                          href={studioHref(`/youtube/playlists/${playlist.playlist_id}`, projectId)}
                        ...
                        >
                          ...
                          <span className="text-base font-semibold truncate" title={playlist.title}>
                            {playlist.title}
                          </span>
                        </Link>
                      ...
                      <div className="text-xs text-gray-500 mb-1">
                        {playlist.item_count ?? 0}개 · {playlist.privacy_status || "-"}
                      </div>
