...
import { voiceApi, modelsApi, scriptApi, taskApi, assetUrl, type Project, type Cut, type ModelInfo } from "@/lib/api";
...
export default function StepVoice({ project, cuts, onUpdate }: Props) {
  ...
  const previewVoice = async () => {
    ...
    try {
      ...
      if (result.path) {
        ...
        const audio = new Audio(`${assetUrl(project.id, result.path)}?t=${Date.now()}`);
  ...
  const playAudio = (cut: Cut) => {
    ...
    const audio = new Audio(assetUrl(project.id, cut.audio_path));
  ...
  const costPerKChars = selectedTts?.cost_value ?? 0.10;
