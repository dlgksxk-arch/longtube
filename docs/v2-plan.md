...
- 채널 증가, 채널 수, 프리셋 수와 무관하게 대본 생성 프롬프트 소스는 단 1개 파일만 사용한다.
- 전역 대본 생성은 반드시 `backend/app/services/llm/base.py`만 사용한다.
- 추가 대본 생성 프롬프트 파일, 채널별 대본 생성 프롬프트, 프리셋별 대본 생성 프롬프트를 절대 만들지 않는다.
- 대본 생성 기본 프롬프트 수정은 `backend/app/services/llm/base.py` 안에서만 허용한다.
...
5. **대본 프롬프트 단일 파일 원칙**: 대본 생성 프롬프트는 `backend/app/services/llm/base.py`만 사용한다. 채널별/프리셋별/모델별 대본 프롬프트 파일을 추가하지 않는다.
6. **존댓말 문서 원칙 유지**: 코드 주석/UI 문구에 반말·농담 금지.
7. **모바일·상용화는 v3.0으로 유예**: v2.x에서는 PC 기반으로만 구현한다. 보안은 v2.x에서도 최소한(암호화+검증)은 지킨다.
...
4. **프롬프트 역할 분리**:
   - 대본 생성 기본 프롬프트 → `backend/app/services/llm/base.py`에서만 관리.
   - 프리셋 입력값(주제·핵심 사항·순서·톤)은 프롬프트 파일이 아니라 런타임 입력값으로만 전달.
...
- 대본 생성 프롬프트는 모델/채널/프리셋별로 분리하지 않고 `backend/app/services/llm/base.py`만 사용한다.
...
- 대본 생성 기본 프롬프트는 `backend/app/services/llm/base.py` 한 파일만 사용한다.
- 채널별, 프리셋별, 모델별 대본 생성 프롬프트 파일을 만들지 않는다.
- 프리셋은 대본 프롬프트 키를 참조하지 않는다. 주제·핵심 사항·순서·톤 같은 런타임 입력값만 보낸다.
- 이미지/TTS/썸네일 관련 프롬프트는 이 원칙의 대상이 아니지만, 대본 생성 프롬프트와 섞지 않는다.
