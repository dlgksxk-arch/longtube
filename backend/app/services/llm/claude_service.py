        ...
        if parsed is None:
            ...
            async with self._client() as client:
                response = await client.messages.create(
                    ...
                    messages=[{
                        "role": "user",
                        "content": self._build_user_prompt(topic, script_config),
                    }],
                    timeout=None,
                )
            ...
            self._save_raw_response(config.get("__project_id"), "script_response", response)
            raw = response.content[0].text
            parsed = self._parse_json(raw)
        else:
            print("[script] timing checkpoint reused; full Claude generation skipped")
