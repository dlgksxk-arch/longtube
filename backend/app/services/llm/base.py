...
- 캐릭터는 전체 컷의 30~50%에만 자연스럽게 등장시킨다 (예: 12컷이면 4~6컷)
- 캐릭터가 등장하는 컷: 도입부(첫 컷), 핵심 장면, 전환, 마무리 등 스토리상 자연스러운 곳
- 캐릭터가 등장하지 않는 컷: 배경 설명, 자료 화면, 비유적 장면, 분위기 컷 등
...
Image prompt character appearance rules (MANDATORY!!):
- Even if a character is provided, do NOT put the character in EVERY cut!
- The character should appear in only 30-50% of cuts (e.g., 4-6 cuts out of 12)
- Cuts WITH character: intro (first cut), key story moments, transitions, ending — wherever it feels natural
- Cuts WITHOUT character: background explanations, data/fact visuals, metaphorical scenes, mood/atmosphere shots
...
画像プロンプトのキャラクター登場ルール（必須!!）:
- キャラクターが提供されても、全カットにキャラクターを入れないこと！
- キャラクターは全カットの30〜50%にのみ自然に登場させる（例：12カットなら4〜6カット）
- キャラクター登場カット：導入、重要シーン、転換、まとめなどストーリー上自然な場所
- キャラクター不在カット：背景説明、データ映像、比喩的シーン、雰囲気カットなど
- ★★★ キャラクター登場カットでは、image_prompt にキャラクターの **形・動作・ポーズ** を直接記述すること ★★★
  例：「a small character wearing a yellow hat holding a magnifying glass, pointing at an oil barrel」
  形態的特徴のみ — 色や画風の単語は入れないこと。
- キャラクターの形状記述を省略すると画像にキャラクターが出ない。
"""
        ...
        if character_description:
            if language == "en":
                style_instruction = (
                    ...
                    f"Rules:\n"
                    f"- The character should appear in only 30-50% of cuts at natural story moments.\n"
                ...
                style_instruction = (
                    ...
                    f"{character_description}\n\n"
                    f"ルール:\n"
                    f"- キャラクターは全カットの30〜50%の自然な場面にのみ登場させる。\n"
                ...
                style_instruction = (
                    ...
                    f"{character_description}\n\n"
                    f"규칙:\n"
                    f"- 캐릭터는 전체 컷의 30~50% 만, 스토리상 자연스러운 위치에만 등장시킵니다.\n"
