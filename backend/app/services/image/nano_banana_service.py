...
FAL_KEY 가 없거나 엔드포인트 호출이 최종 실패하면 Flux Dev 로 투명 폴백해서
"완전 먹통" 상황을 피합니다. 폴백이 일어나면 로그에 명확히 남습니다.
"""
...
class NanoBananaService(BaseImageService):
    ...
    async def generate(
        ...
        try:
            ...
            print(
                f"[nano-banana] {self.model_id} 호출 실패 → Flux Dev 폴백으로 전환. "
                f"원인: {e}"
            )
            fallback = FluxService("flux-dev")
            return await fallback.generate(prompt, width, height, output_path, reference_images)
