...
> 어떤 엔트리도 git 커밋에 머지된 적 없음. 아이디어만 `docs/archive/v2/v2-plan.md`
...
### 전 채널 본편 렌더·자막·채널 운영 정리

- 전 채널 본편 조립을 공통 FFmpeg 렌더러로 통합하고 무음 구간을 절반으로 줄인다.
- 영상 생성 단계의 중복 본편 병합을 제거하고 실제 렌더 진행률을 작업대에 반영한다.
- 본편 좌우 상단 제목·채널명 레이아웃과 CH3 일본어 제목 검증을 적용한다.
- 남은 대본의 한국식 예능 자막 동기화 도구와 검증을 추가한다.
- 제작 파이프라인의 OpenAI 차단을 유지하면서 채널 댓글 번역·답변 기능만 허용한다.
- CH4 워크북의 절대 worksheet relationship 경로를 정상 처리한다.
...
- TTS 음성을 `cut_video_duration`에 강제 보정하지 않고 원본 길이로 저장한다.
- 영상 컷 길이를 `실제 TTS 길이 + 앞 0.3초 + 뒤 0.3초`로 계산한다.
- FFmpeg/fal/Kling mux 경로에서 오디오를 0.3초 지연시켜 앞 여백을 만든다.
- 기존 컷 mp4의 길이가 새 목표 길이와 다르면 재사용하지 않고 재생성한다.
- 앱 표기를 `V3.2`로 올렸다.
